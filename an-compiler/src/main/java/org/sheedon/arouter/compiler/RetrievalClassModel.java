...
public class RetrievalClassModel {
    ...
    // 请求路由泛型记录
    private BRGenericsRecord record;
    // 泛型对照类，当前类与父类
    ...
    public void addGenericsRecord(String typeName, String entityClassName) {
        BRGenericsRecord record = getRecord();
     ...
     * 获取泛型记录
     */
    public BRGenericsRecord getRecord() {
        if (record == null) {
            record = new BRGenericsRecord();
        }
        return record;
    }
     ...
     * @param record 泛型记录
     */
    public void bindGenericsRecord(BRGenericsRecord record) {
        this.record = record;
    }
