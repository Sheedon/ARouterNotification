...
/**
 * BindRouterCard 类检索者
 * 搜索从当前类开始，层级向上，检索至{@link BindRouterCard}
 * 将T所对应的「实体类全类名」绑定到泛型类型上，
 ...
 */
public class BindRouterClassSearcher {

    private final Map<String, RetrievalClassModel> classMap = new HashMap<>();
    // 目标全类名
    private final static String CLASSNAME = BindRouterCard.class.getCanonicalName();
    // java包
    ...
    public RetrievalClassModel searchClassGenerics(TypeElement element, Types types, Messager messager) {
        ...
        BRGenericsRecord record = cloneBySuperRecord(classModel.getRecord());
    ...
    public RetrievalClassModel getRetrievalClassByClassName(String className) {
     ...
     * @return 当前类的泛型处理记录
     */
    private BRGenericsRecord cloneBySuperRecord(BRGenericsRecord record) {
        if (record == null) {
